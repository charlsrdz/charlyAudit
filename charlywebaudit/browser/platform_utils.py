...
def needs_virtual_display() -> bool:
    ...
    típico de un servidor/CI/contenedor sin entorno gráfico. charlyWebAudit
    usa `headless: false` (más confiable para pruebas reales con
    interacción visual que el modo headless puro), lo que requiere un
    display real o uno virtual (Xvfb). En macOS/Windows, correr de forma
    interactiva siempre implica una sesión gráfica real — esta condición
    nunca aplica ahí."""
    if not is_linux():
        return False
